...
rst=stmt.executeQuery("select productname,productprice,quantity,productid  from specialproduct  where productname='"+pname+"' ");
...
out.println(e);
 ...
 // out.print("product  amount"+productAmt);
  ...
  <input type="hidden" name="storephone"  value="<%=session.getAttribute("user")%>"/>
  ...
  <%  
	...
	int x=stmt.executeUpdate("insert into orderlist (productname,productCast,quantity,totalAmount,address,cardHolderName,cardtype,cardNum,cvv,customerid)values('"+t1+"','"+t2+"','"+t3 +"','"+t4+"','"+t5+"','"+t6+"','"+t7+"','"+t8+"','"+t9+"','"+session.getAttribute("cid")+"')");
	if(x!=0){
	...
	window.open("viewMyOrders.jsp","_self");
	</script>
